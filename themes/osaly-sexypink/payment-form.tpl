...
{else}
	...
	<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std">
		...
		{/foreach}
		<p class="text">
			<label for="name">{l s='E-mail address'}</label>
			<input type="text" id="name" name="name" value="{if isset($smarty.post.from)}{$smarty.post.from|escape:'htmlall':'UTF-8'|stripslashes}{/if}" />
		</p>
		<p class="text">
